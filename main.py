...
################################################################################
# Dieser Code wurde mithilfe der offiziellen NVIDIA Beispielanwendungen
# entwickelt und basiert auf den Beispielen "deepstream-rtsp-in-rtsp-out" und
# "deepstream-nvdsanalytics".

# Quelle: https://github.com/NVIDIA-AI-IOT/deepstream_python_apps
################################################################################
...
# src_pad_buffer_probe will extract metadata received on nvtiler sink pad
...
def main(args):
    ...
    # create source_bins for all input sources
    ...
    streammux.set_property('batched-push-timeout', 1/33)
    ...
    # POSTOSD                   #
    # # # # # # # # # # # # # # #
    print("Creating nvvidconf_postosd \n ")
    ...
    # INPUT: multiple sources -> streammux -> queue1 -> pgie -> queue2 -> tracker -> queue3 -> nvanalytics -> queue4 ->
    # -> tiler -> queue5 -> nvvidconv -> queue6 -> nvosd -> queue7 -> nvvidconv_postosd -> caps -> encoder --
    #                                                                                                       |
    #                                      OUTPUT: RTSP stream                              sink <- rtpay <-|
    #                                                                                                       |
    #                                      OUTPUT: Write to file                sink <- mux <- codecparse <--
    ...
    if RTSP_OUT:
        ...
        factory.set_launch(
            "( udpsrc name=pay0 port=%d buffer-size=524288 caps=\"application/x-rtp, media=video, clock-rate=90000, encoding-name=(string)%s, payload=96 \" )" % (
                udpsink_port_num, CODEC))
        factory.set_shared(True)
        server.get_mount_points().add_factory("/output-stream", factory)

        print("\n *** DeepStream: Launched RTSP Streaming at rtsp://" + socket.gethostbyname(socket.gethostname()) + ":" + str(rtsp_port_num) + "/output-stream ***\n\n")

    # nvdsanalytics source pad allocation
    nvanalytics_src_pad = nvanalytics.get_static_pad("src")
    if not nvanalytics_src_pad:
        sys.stderr.write(" Unable to get src pad \n")
    else:
        nvanalytics_src_pad.add_probe(Gst.PadProbeType.BUFFER, src_pad_buffer_probe, 0)
